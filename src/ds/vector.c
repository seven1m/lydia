...
vector_p create_vector(){
	vector_p vec = (vector_p)GC_MALLOC(sizeof(struct vector));
	vec->data = (void**)GC_MALLOC(sizeof(void*)*BASE_CAP);
	vec->sizes = (int*)GC_MALLOC(sizeof(int)*BASE_CAP);
	vec->capacity = BASE_CAP;
	vec->length = 0;
	vec->destructor = GC_free;
	return vec;
}
...
void check_length(vector_p vec){
	if(vec->length >= vec->capacity){
		...
		vec->data = (void**)GC_REALLOC((void*)vec->data, 
									vec->capacity*sizeof(void*));
		vec->sizes = (int*)GC_REALLOC((void*)vec->sizes, 
