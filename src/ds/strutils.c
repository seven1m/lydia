...
char * saferead(FILE * f){
	...
	len = ftell(f);
	fseek(f, 0, SEEK_SET);
	buf = (char*)GC_MALLOC(sizeof(char)*(len+1));
...
char ** str_split(char * str, char * delim, int * size){
	...
	while(tok){
		if(len >= cap){
			cap = len * 1.5;
			array = (char**)GC_REALLOC(array, sizeof(char*) * cap);
		}
		slen = strlen(tok);
		if(slen > 0){
			array[len] = (char*)GC_MALLOC(slen+1);
...
stringbuf * make_buffer(int capacity){
	stringbuf * buf = (stringbuf*)GC_MALLOC(sizeof(stringbuf));
	buf->length = 0;
	buf->capacity = capacity;
	buf->str = (char*)GC_MALLOC(sizeof(char)*capacity);
...
void realloc_if_needed(stringbuf * buf){
	if(buf->capacity <= buf->length){
		...
		buf->str = GC_REALLOC(buf->str, buf->capacity);
	}
}
